...
for event in events:
    ...
    for game in event["games"]:
        ...
        for result in game["results"]:
            ...
            f.write( ",".join([event["gender"], event["name"].encode("utf-8").strip().decode("utf-8"), game["location"], str(game["year"]), result["medal"], result["name"] or "null", result["nationality"], str(result["result"]) or "null"]) + "\n")
